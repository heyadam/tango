...
Split-pane design-prototype app: a direct-manipulation design canvas on the left (screens of absolutely-positioned shadcn/Tailwind components — drag, resize, text-edit), an interactive xterm.js terminal on the right backed by a real shell via `node-pty` (auto-running `claude` or `codex`), and an optional iOS Simulator mirror in a third pane. The core loop: **import** mobile code onto the canvas (agent-mediated) → **edit** directly → **live-preview** natively on the simulator in <1s via the preview-host app (no rebuild) → **export** deterministically to SwiftUI and build/launch (`export_run`, no LLM).

Stack: **Next.js 16 (App Router) + React 19 + Tailwind v4**, custom Node server, WebSocket bridges to `node-pty` / the design spec / the preview host, in-process MCP server giving the terminal agent tools to drive the canvas and the iOS toolchain, and a small committed **SwiftUI preview-host app** ([preview-host/](preview-host/)). Will eventually be packaged as an Electron app — keep that path open (no Vercel-only assumptions, no edge runtime).
...
3. `null` — picker shows blocking; PTY refuses to spawn (`/ws/terminal` closes with 4001). Source `'unset'`.
...
User-level state lives at `~/.tango/state.json` (keys: `lastWorkspace`, `terminalAgent`). Electron will swap `os.homedir()` for `app.getPath('userData')` here; don't introduce other paths to that file.
...
- **Bridge to the terminal via `submitToTerminal` for intentional asks** (Import, Send) — it writes text + `\r` as two PTY writes. Never write a bare path into the PTY (the shell executes it on the next Return); prefix informational pings with `#`.
...
- **Strict Mode is on.** The terminal effect runs twice in dev → one harmless "WebSocket is closed before the connection is established" warning per mount, and the agent CLI boots twice on first load (the first PTY is killed by the Strict-Mode cleanup). Don't "fix" by disabling Strict Mode.
...
- [server.ts](server.ts) — Next + WS upgrade routing + MCP mount + boot-time workspace resolution + spec hydration + exit flush
- [src/server/pty.ts](src/server/pty.ts) / [ptyEnv.ts](src/server/ptyEnv.ts) / [terminalAgent.ts](src/server/terminalAgent.ts) — terminal bridge + agent launch commands
...
- [src/lib/uiImportPrompt.ts](src/lib/uiImportPrompt.ts) — the Import button's prompt to the terminal agent
