...
- **Controller agent** ([src/app/api/agent/route.ts](src/app/api/agent/route.ts) + [src/components/AgentTrigger.tsx](src/components/AgentTrigger.tsx) + [src/components/AgentCursorOverlay.tsx](src/components/AgentCursorOverlay.tsx) + [src/server/agentCursorBridge.ts](src/server/agentCursorBridge.ts)): a small "Tell the agent what to do…" input lives in the SketchPanel toolbar. Submitting POSTs to `/api/agent`, which spins up a Vercel AI SDK `streamText` loop against `gpt-5.5` (model + key in [src/lib/ai.ts](src/lib/ai.ts), `reasoningEffort: 'low'`), connects an `@ai-sdk/mcp` HTTP client to *this same* `/mcp` endpoint, and exposes a **filtered** subset of tools to the model. The browser side is `AgentTrigger` (uses `useChat` from `@ai-sdk/react`) for the goal box + log panel, and `AgentCursorOverlay` for the visible cursor sprite + DOM event dispatch + terminal-write seam. Load-bearing details:
  ...
  - **`AgentCursorOverlay` is gated on `agentOpen` in [page.tsx](src/app/page.tsx)** — when the agent sidebar is collapsed the overlay unmounts (cursor hidden, `/ws/agent-cursor` torn down). The sprite is `position: fixed`, `pointerEvents: 'none'`, `z-index: 99999`, and tagged `data-agent-cursor` so it can be filtered out of `dom_inspect` results in the future if needed.
...
- **Simulator panel** ([src/server/sim.ts](src/server/sim.ts) + [src/app/api/sim/status/route.ts](src/app/api/sim/status/route.ts) + [src/components/SimulatorPanel.tsx](src/components/SimulatorPanel.tsx)): the right-most aside iframes [serve-sim](https://github.com/EvanBacon/serve-sim)'s preview UI so the user can drive a booted iOS Simulator without leaving tango. Single global helper per server run — not per-WS, not per-workspace, because `serve-sim` is a system-level resource that sees whatever simulator the user has booted via Xcode/`simctl`. Load-bearing details:
  ...
  - **Discriminated union state.** `SimStatus = { phase: 'unsupported' } | { phase: 'starting' } | { phase: 'ready'; url } | { phase: 'error'; message }` — never a struct with optional fields. Keeps the React panel's branches honest and means `ready` always carries a non-null `url`. Extend the union if you add new states; do not loosen it back to optional fields.
  - **Process-group teardown.** `spawn('npx', ['serve-sim'], { detached: true })` puts the child in its own pgid; `stopSimHelper` calls `process.kill(-pid, 'SIGTERM')` (negative pid = process group) with a 2s SIGKILL escalation. Without this, `child.kill('SIGTERM')` only signals the `npx` shim and leaves the actual `serve-sim` Swift binary as an orphan reparented to launchd. SIGINT/SIGTERM/`exit` handlers in [server.ts](server.ts) all run `stopSimHelper`.
  - **Localhost-only URL parse.** `parsePreviewUrl` requires a `\n`-terminated banner (chunk-boundary safety against partial reads) AND validates the URL's hostname is `localhost` / `127.0.0.1` / `::1` before returning it. Defense-in-depth: a future compromised `serve-sim` printing `Preview at https://attacker.example` is rejected, not iframed.
  - **Iframe is sandboxed.** `sandbox="allow-scripts allow-same-origin allow-forms"`; deliberately no `allow-top-navigation`, no `allow-popups`. The iframe content is from an `npx`-fetched package whose UI we render in our origin's child frame — keep the blast radius small.
  - **No simBus.** Unlike `terminalBus` / `canvasBus` / `agentCursorBridge`, there's no duplex traffic from tango's React tree into the simulator. The iframe handles its own MJPEG/WS directly to serve-sim. If we ever need terminal-Claude or the controller agent to drive the simulator (e.g. tap, swipe, rotate), reach for an MCP tool that calls serve-sim's HTTP API instead of inventing a bus.
  - **Known limitation: loopback only.** `serve-sim` binds to localhost; if you open tango at `http://10.0.0.5:3000` from another machine on the LAN, the iframe will try `http://localhost:3200` against the *client's* loopback and fail silently. Not fixed today; the panel just shows a broken iframe in that case.
...
1. **Now:** terminal auto-launches `claude --dangerously-skip-permissions` in `WORKSPACE_DIR`. MCP server gives it canvas tools (read/write the scene) and agent UI tools (`dom_inspect` + cursor + `terminal_type`). A small gpt-5.5 controller agent in the toolbar takes user goals, finds UI targets via `dom_inspect`, moves the visible cursor, and delegates the actual brainwork to terminal-Claude via `terminal_type`. Designer mode is duplex. Simulator sidebar iframes serve-sim for live iOS device preview on darwin. PNG-on-disk + comment-line ping is still around as a manual vision channel.
2. **Next:** richer canvas tools (image insertion, select-and-modify by element ID, scoped diffs). A `screenshot_canvas` tool that returns rendered pixels so terminal-Claude can *see* the sketch (not just parse element JSON). Tool-call verification (e.g. `cursor_click` returns "landed on `<button>X</button>`" so the model self-corrects). Other agents (codex). Per-tab / per-project workspaces. Possibly recording the bus traffic for replay. MCP tools that drive the simulator (tap / swipe / rotate / install) so terminal-Claude can iterate on a mobile UI without the human in the loop. Same-origin mount of serve-sim's UI via `simMiddleware` to drop the cross-port iframe and unblock LAN dev.
...
- [src/server/sim.ts](src/server/sim.ts) — process-group child manager for `npx serve-sim`; discriminated-union status + chunk-buffered, localhost-only stdout parse + SIGKILL escalation
- [src/app/api/sim/status/route.ts](src/app/api/sim/status/route.ts) — GET wrapper for `getSimStatus()`
- [src/components/SimulatorPanel.tsx](src/components/SimulatorPanel.tsx) — sandbox-iframed serve-sim preview; AbortController-cleaned poll loop with 1s→2.5s backoff
