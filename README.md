...
4. **Export & Run** — one click deterministically writes the design into your own SwiftUI sources and builds/installs/launches. Imported screens get their View's `var body` replaced in place, so the running app shows the design with no wiring step. No LLM in that path.
...
- **Export & Run** splices each linked screen's `var body` into its own View struct in place (marked `// tango:body` — that body is design-managed and regenerated on every export; the rest of the file stays yours). Canvas-born screens become new `<Name>Screen.swift` files at the app source root (Xcode 16 filesystem-synchronized projects pick them up automatically; older projects need a one-time drag into the target, surfaced as `manual-add-required`). Hand-edited files are skipped, never clobbered, and pre-export originals land in `.tango/export-backup/`.
