...
- \`duplicate_ui_screen\` — copy a screen as a new screen (node ids remapped automatically); the fast first step for variations: duplicate, then patch deltas
...
- \`update_ui_nodes\` — many node patches in ONE call; the cheap delta path for restyles/rearranges (emit only fields that change)
- \`update_ui_screen\` — patch a screen's title/frame (id immutable)
...
\`sourceFile\` is import provenance AND the export target: \`export_run\` rewrites a linked screen's View body in place inside that exact file, so dropping the field silently changes the next export from an in-place rewrite into a brand-new file. Set it only when the screen mirrors a real file, and preserve it when re-emitting an existing screen. \`sourceHash\` is the file's content fingerprint stamped by tango's import engine — preserve it alongside sourceFile and NEVER fabricate or alter it (the canvas uses it to show "code changed since import", and export refuses to overwrite hand-edited files based on it). Screens WITHOUT a sourceFile export as a new \`<Pascal(id)>Screen.swift\` (name derived at export time, then linked by the restamp) — don't try to record export filenames here.
...
- \`get_ui_layers\` — compact z-ordered outline (screens → nodes back-to-front, each with \`z\`, \`id\`, \`type\`, \`text\`, \`rect\`). Cheap way to grab the right node id and see stacking; use \`get_ui_mock\` when you need full styling/props (pass \`screenId\` to read just one screen).
...
- \`update_ui_nodes({ patches })\` — many node patches in ONE call; prefer this for any multi-node restyle/rearrange (emit only the fields that change).
- \`duplicate_ui_screen({ screenId, newScreenId, newTitle? })\` — instant screen copy with node ids remapped; duplicate-then-patch is the fast variation path.
- \`update_ui_screen({ screenId, title?, frame? })\` — patch screen-level fields (id immutable).
